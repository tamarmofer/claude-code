...
| [outlook-connect](./outlook-connect/) | Connect to Microsoft Outlook for email and calendar management via Microsoft Graph API | **Commands:** `/auth`, `/read-emails`, `/send-email`, `/search-emails`, `/calendar` - Full Outlook integration<br>**Agent:** `email-analyzer` - Analyze and prioritize inbox emails |
